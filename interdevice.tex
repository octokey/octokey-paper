...
the user's other devices (for mutual authentication of channels), configuration (e.g. URL of the key
fragment server), information on which devices are paired (for key revocation purposes), a list of
all the services they have logged in to (for username autofill), and so on.
...
The key fragment store also needs access to this database in order to correctly authenticate clients
and process revocation requests. Although the key fragment store cannot authenticate with services
on behalf of the user, it can track the user's login activity, which may be a privacy concern if the
key fragment store is hosted by a third party. Thus, users may choose to operate their own key
fragment store if they wish, although doing so will require technical expertise (see also
section~\ref{sec:store-security}).
