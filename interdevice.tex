...
We propose adding an expiry date to every user keypair, by default four months after the key's
creation date. The expiry date is signed with the private key, to prevent tampering, and included in
the public key that is sent to services. Services must not accept public keys without associated
expiry date, or keys that have expired.
